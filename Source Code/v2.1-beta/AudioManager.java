 ...
 */
public class AudioManager<T extends AudioManager<T>> extends Formatter<AudioManager<T>> {
     ...
     */
    AudioManager(CoreState coreState) {
        super(coreState);
    }
    ...
    public T load(String id, String path) {
     ...
     */
    @SuppressWarnings("unchecked")
    public T startPlaying(String id, String location, int playTimeMillis) {
        load(id, location);
        setMasterVolume();
        play(id);
        if (playTimeMillis > 0)
